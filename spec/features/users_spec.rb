...
RSpec.feature "Users", type: :feature do
  ...
  describe 'User show page' do
    ...
    scenario 'When I click on the button, I am redirected to the user\'s posts index page' do
      ...
      expect(current_path).to eq(user_posts_path(@user3))
    end

    scenario 'When I click on a post, I am taken to that post\'s show page' do
      visit user_posts_path(@user3, @user3.posts)
      click_link @user3.posts.first.title
      expect(current_path).to eq(user_post_path(@user3, @user3.posts.first ))
    end
